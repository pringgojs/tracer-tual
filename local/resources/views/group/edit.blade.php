    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card p-20">
                ...
                <form class="form-material m-t-40" action="{{url('group/'.$group->id)}}" method="post">
                    {!! csrf_field() !!}
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" required value="{{$group->name}}" name="name" class="form-control" placeholder="">
                        </div>
                        <div class="form-group">
                            <label class="control-label">Description <br><small>used to provide a description of the group</small></label>
                            <input type="text" id="desc" value="{{$group->description}}" required name="description" class="form-control" placeholder="">
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" id="link" required value="{{$group->link}}" name="link" class="form-control" >
                        </div>
                        <div class="form-group">
                            <label>Order number <br><small>used to sort groups</small></label>
                            <input type="number" min="1" max="100" required id="order_number" value="{{$group->order_number}}"  name="order_number" class="form-control" placeholder="">
                        </div>
                        <div class="form-group">
                            <label>Icon</label>
                            <?php
                            ...
                            <div id="element-icon">
                                @foreach($icons as $icon)
                                    <button type="button" class="btn btn-sm btn-circle @if($icon == $group->icon) btn-warning @endif" onclick="selectIcon('{{$icon}}')"><i class="{{$icon}}"></i></button>
                                @endforeach
                                <input type="hidden" name="icon" value="{{$group->icon}}" readonly id="icon">
                            </div>
                        </div>
                        <div class="form-group">
